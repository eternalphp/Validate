...
class Validator {
	...
	public static function isLegalLength($string, $length, $charset = 'utf8') {
		var_dump(String::strlen($string, $charset));
	 ...
	 * 验证字符串的长度是否在所在范围
	 * 
	 * @param string $value  需要被验证的籽符串
	 * @param array $range  验证的范围
	 * @return boolean
	 */
	public static function between($value,$range = array()){
	 ...
	 * 验证字符串的最小长度
	 * 
	 * @param string $value  需要被验证的字符串
	 * @param int $minlength  验证的范围
	 * @return boolean
	 */
	public static function minlength($value,$minlength){
		if(strlen($value) >= intval($minlength)){
			return true;
		}else{
			return false;
		}
	...
	public static function maxlength($value,$maxlength){
		if(strlen($value) <= intval($maxlength)){
			return true;
		}else{
			return false;
		}
	 ...
	 * 
	 * @param string $value  需要被验证的字符串
	 * @param string $otherValue  被比较的字符串
	 * @return boolean
	 */
	public static function compare($value,$otherValue){
		if($value == $otherValue){
			return true;
		}else{
			return false;
